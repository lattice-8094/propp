"""
propp_charnet.py - Character Network Visualization for propp outputs
...
Functions:
    quickplot(filename, output_dir=None, top_n=10) - Process a single book
    all_plot(folder_path, output_dir=None, top_n=10) - Batch process all books in folder
"""
...
def parse_book_file(book_path: Path) -> Dict:
    ...
    The .book file contains Python dict literals, one per line.
    Each dict has: id, count (with occurrence), mentions (proper/common/pronoun), etc.
...
def quickplot(
    ...
    Example (notebook):
        >>> from propp_charnet import quickplot
        >>> metrics, fig, G = quickplot("1887_Guy-de-Maupassant_Le-Horla")
        >>> fig.show()
    """
...
if __name__ == "__main__":
    ...
    if args.batch:
        all_plot(args.input, args.output_dir, args.top_n)
    else:
        quickplot(args.input, args.input_dir, args.output_dir, args.top_n)
